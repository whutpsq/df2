...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(
        ...
        self.default_eval_range = default_eval_range
        from nuscenes.eval.detection.config import config_factory

        self.eval_detection_configs = config_factory(self.eval_version)
        self._patch_eval_detection_configs()
    ...
    def _patch_eval_detection_configs(self):
        """Adapt nuScenes detection eval config for custom class names."""
        self.eval_detection_configs.class_names = list(self.CLASSES)
        for class_name in self.CLASSES:
            self.eval_detection_configs.class_range.setdefault(
                class_name, self.default_eval_range
            )
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesDataset.DefaultAttribute.get(name, "")
                else:
    ...
    def evaluate_custom_bbox(self, results, dist_ths=(0.5, 1.0, 2.0, 4.0)):
        """Evaluate custom detection classes by center-distance matching."""
        metrics = {}
        class_aps = []
        ...
        for sample_id in range(len(self)):
            ...
            gt_boxes = ann_info["gt_bboxes_3d"].gravity_center[:, :2].numpy()
            gt_labels = ann_info["gt_labels_3d"]
            valid = gt_labels >= 0
            gt_by_sample.append((gt_boxes[valid], gt_labels[valid]))

        for class_id, class_name in enumerate(self.CLASSES):
            num_gt = sum(np.sum(labels == class_id) for _, labels in gt_by_sample)
            class_ap_values = []
            ...
            for dist_th in dist_ths:
                ...
                for sample_id, result in enumerate(results):
                    ...
                    if not np.any(class_mask):
                        continue
                    centers = result["boxes_3d"].gravity_center.numpy()[class_mask, :2]
                    scores = result["scores_3d"].numpy()[class_mask]
                    for center, score in zip(centers, scores):
                        detections.append((sample_id, float(score), center))
                ...
                matched = {
                    sample_id: np.zeros(np.sum(labels == class_id), dtype=bool)
                    for sample_id, (_, labels) in enumerate(gt_by_sample)
                }

                for det_id, (sample_id, _, center) in enumerate(detections):
                    gt_centers, gt_labels = gt_by_sample[sample_id]
                    gt_centers = gt_centers[gt_labels == class_id]

                    if gt_centers.shape[0] == 0:
                        fp[det_id] = 1.0
                        continue

                    distances = np.linalg.norm(gt_centers - center[None, :], axis=1)
                    match_id = int(np.argmin(distances))
                    if distances[match_id] <= dist_th and not matched[sample_id][match_id]:
                        tp[det_id] = 1.0
                        matched[sample_id][match_id] = True
                    else:
                        fp[det_id] = 1.0
                ...
                if num_gt == 0:
                    ...
                    recall = recalls[-1] if recalls.size > 0 else 0.0

                metrics[f"object/{class_name}_ap_dist_{dist_th}"] = float(ap)
                metrics[f"object/{class_name}_recall_dist_{dist_th}"] = float(recall)
                if not np.isnan(ap):
                    class_ap_values.append(ap)

            metrics[f"object/{class_name}_ap"] = (
                float(np.mean(class_ap_values)) if class_ap_values else float("nan")
            )
            if class_ap_values:
                class_aps.append(np.mean(class_ap_values))

        metrics["object/map"] = float(np.mean(class_aps)) if class_aps else 0.0
        return metrics
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if "boxes_3d" in results[0]:
            if not set(self.CLASSES).issubset(set(self.NUSCENES_CLASSES)):
                metrics.update(self.evaluate_custom_bbox(results))
                return metrics
