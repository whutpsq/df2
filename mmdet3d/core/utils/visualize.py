    ...
    *,
    ...
    if bboxes is not None and len(bboxes) > 0:
        ...
        for index in range(coords.shape[0]):
            name = classes[labels[index]] if labels is not None else None
            line_color = color or OBJECT_PALETTE.get(name, (255, 255, 255))
            for start, end in [
                (0, 1),
                (0, 3),
                (0, 4),
...
def visualize_lidar(
    ...
    *,
    ...
    if bboxes is not None and len(bboxes) > 0:
        coords = bboxes.corners
        if hasattr(coords, "detach"):
            coords = coords.detach().cpu().numpy()
        coords = coords[:, [0, 3, 7, 4, 0], :2]
        for index in range(coords.shape[0]):
            name = classes[labels[index]] if labels is not None else None
            plt.plot(
                coords[index, :, 0],
